...
/// </summary>
public sealed class ModbusEndpoint(ILogger log, IReadOnlyDictionary<int, int> regBase)
{
    ...
    public bool Open(string ip, int port, byte unit)
    {
        try
        {
            ...
            if (_client.IsConnected) log.LogInformation("Modbus connected {Ip}:{Port} unit {Unit}", ip, port, unit);
            return _client.IsConnected;
        }
        catch (Exception ex)
        {
            log.LogWarning("Modbus connect failed: {Msg}", ex.Message);
            _client = null;
            return false;
        }
    }
    ...
    public ushort[]? ReadRegs(int db, int regOffset, int count)
    {
        try
        {
            if (_client?.IsConnected != true || !regBase.TryGetValue(db, out var b)) return null;

            int abs = b + regOffset;
            log.LogInformation("MODBUS  FC03 read   addr 4{Addr:D5}  count {Count}", abs + 1, count);
        ...
        catch (Exception ex) { log.LogDebug("Modbus read DB{Db} failed: {Msg}", db, ex.Message); return null; }
    }
    ...
    public bool WriteRegs(int db, int regOffset, ushort[] values)
    {
        try
        {
            if (_client?.IsConnected != true || !regBase.TryGetValue(db, out var b)) return false;

            int abs = b + regOffset;
            log.LogInformation("MODBUS  FC16 write  addr 4{Addr:D5}  = [{Vals}]", abs + 1, string.Join(",", values));
            var s = new short[values.Length];
            for (int i = 0; i < values.Length; i++) s[i] = (short)values[i];
            _client.WriteMultipleRegisters(_unit, abs, s);
            return true;
        }
        catch (Exception ex) { log.LogWarning("Modbus write DB{Db} failed: {Msg}", db, ex.Message); return false; }
    }
}
